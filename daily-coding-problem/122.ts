...
function get_max_coins(matrix: number[][]): number {
  const m_length: number = matrix.length
  const m_width: number = matrix[0].length

  function get_max_coins_helper(
    x: number, 
    y: number, 
    max: number): number {
...
1 5 3 1
*/
console.log(get_max_coins([[0, 3, 7, 4], [2, 0, 0, 4], [1, 5, 3, 1]]))
/* test case:
0 3 1 1 1 5
2 0 0 4 3 1
*/
console.log(get_max_coins([[0, 3, 1, 1, 1, 5], [2, 0, 0, 4, 3, 1]]))
